...
const FarmCard: React.FC<FarmCardProps> = ({ farm, removed, pizzaPrice, bnbPrice, ethereum, account }) => {
  ...
  const earnLabel = farm.dual ? farm.dual.earnLabel : 'ORI'
  ...
  return (
    <FCard>
      {farm.tokenSymbol === 'ORI' && <StyledCardAccent />}
      <CardHeading
        lpLabel={lpLabel}
    ...
    {!removed && (
        <Flex justifyContent="space-between" alignItems="center">
          ...
          <Text bold style={{ display: 'flex', alignItems: 'center', color:'#FFFFFF' }}>
            {farm.apy ? (
              <>
                <ApyButton
                  lpLabel={lpLabel}
                ...
                />
                {farmAPY}%
                {/* {(farmImage==="dcash-bnb") ? "1.54" : ""}
                {(farmImage==="dcash-busd") ? "1.15" : ""}
                {(farmImage==="dcash-degg") ? "1.15" : ""}
                {(farmImage==="bnb-busd") ? "0.38" : ""}
                {(farmImage==="bnb-cake") ? "0.38" : ""}
                {(farmImage==="bnb-eth") ? "0.38" : ""} */}
              </>
            ) : (
